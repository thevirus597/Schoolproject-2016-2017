...
$cust_query = "SELECT * FROM rs_customer WHERE rs_cust_name = '$voornaam' AND rs_cust_surname = '$familienaam'";
...
if ($cust_id != $car_id) {
  ...
  $sql = mysqli_query($conn,"INSERT INTO rs_service(rs_service,rs_custid,rs_userid,rs_carplate,rs_serviceinfo,rs_price,rs_date)
    VALUES('$info','$cust_id','$monteur','$kenteken','$reparatie','$prijs','$datum')");
}else {
  $cust_query = mysqli_query($conn,"UPDATE rs_customer SET rs_cust_name = '$voornaam', rs_cust_surname= '$familienaam' WHERE id = '$cust_id'");
  $sql = mysqli_query($conn,"INSERT INTO rs_service(rs_service,rs_custid,rs_userid,rs_carplate,rs_serviceinfo,rs_price,rs_date)
    VALUES('$info','$cust_id','$monteur','$kenteken','$reparatie','$prijs','$datum')");
    if($sql) {
  ...
  else if ($custcheck == 0 && $carcheck == 0 ) {
...
}elseif($custcheck > 0 && $carcheck == 0){//Als kalnt voorkomt, maar auto niet
  while ($row = mysqli_fetch_assoc($result)) {
    $cust_id = $row['id'];
