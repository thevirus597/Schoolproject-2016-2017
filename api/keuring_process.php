...
if ($carcheck > 0 && $custcheck > 0) { //als auto al voorkomt in DB
  ...
  $car_query = mysqli_query($conn,"UPDATE rs_car SET rs_carkeur = '$keuring_date' WHERE rs_car_carplate = '$kenteken'"); //update keuringsdate
  $sql = mysqli_query($conn,"INSERT INTO rs_service(rs_service,rs_custid,rs_userid,rs_carplate,rs_serviceinfo,rs_price,rs_date)
    VALUES('$info','$cust_id','$monteur','$kenteken','gekeurd','$prijs','$datum')");
    if($sql) {
      ...
      echo mysqli_error($conn);
    ...
    else   if ($custcheck == 0 && $carcheck == 0 ) {//als auto niet voorkomt in DB
        ...
        $car_query = mysqli_query($conn,"INSERT INTO rs_car(rs_car_carplate,rs_cust_id,rs_car_carmake,rs_car_carmodel,rs_car_carchas,rs_car_caryear,rs_carkeur)
        VALUES('$kenteken','$cust_id','$merk','$model','$chassis','$bouwjaar','$keuring_date')");
...
}else if($custcheck > 0 && $carcheck == 0){//Als kalnt voorkomt, maar auto niet
  while ($row = mysqli_fetch_assoc($result)) {
    $cust_id = $row['id'];
